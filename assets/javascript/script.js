...
$(document).ready(function () {
  $(".sayAboutSlider").slick({
    ...
    speed: 1000,
    prevArrow:
      '<button type="button" class="single-slick-arrow slick-prev"><img src="assets/images/svg/white-left-arrow.svg" alt="left-black-arrow"></button>',
    nextArrow:
      '<button type="button" class="single-slick-arrow slick-next"><img src="assets/images/svg/white-right-arrow.svg" alt="right-half-arrow"></button>',
    responsive: [
      {
        breakpoint: 991,
        settings: {
...
document.querySelectorAll(".faq-question").forEach((button) => {
  button.addEventListener("click", () => {
    ...
    // Agar clicked item pehle se active hai toh close karo
    if (faqItem.classList.contains("active")) {
      faqItem.classList.remove("active");
      faqAnswer.style.maxHeight = null;
    } else {
      // Agar dusre open items ko close karna chahte ho:
      document.querySelectorAll(".faq-item").forEach((item) => {
        item.classList.remove("active");
        item.querySelector(".faq-answer").style.maxHeight = null;
      });

      // Current waale ko open karo
...
document.querySelectorAll(".faq-question").forEach((button) => {
  button.addEventListener("click", () => {
    ...
    // Agar clicked accordion pehle se open hai toh use close karo
    if (faqItem.classList.contains("active")) {
      faqItem.classList.remove("active");
      faqAnswer.style.maxHeight = null;
    } else {
      // Optional: Pehle se khule hue baki sabhi FAQs ko close karne ke liye
      document.querySelectorAll(".faq-item").forEach((item) => {
        item.classList.remove("active");
        item.querySelector(".faq-answer").style.maxHeight = null;
      });

      // Sirf current clicked accordion ko open karne ke liye
      faqItem.classList.add("active");
      faqAnswer.style.maxHeight = faqAnswer.scrollHeight + "px";
    }
  });
...
document.querySelectorAll(".faq-question").forEach((button) => {
  button.addEventListener("click", () => {
    ...
    // Agar already open hai toh close karo
    if (faqItem.classList.contains("active")) {
      faqItem.classList.remove("active");
      faqAnswer.style.display = "none";
    } else {
      // Pehle se khule hue baki sabhi ko close karo
      document.querySelectorAll(".faq-item").forEach((item) => {
        item.classList.remove("active");
